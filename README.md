...
```
usage: read-skipfile-results [-h] --group-name GROUP_NAME --run-count RUN_COUNT
                             --username USERNAME [--builds-file BUILDS_FILE] [--debug]
                             [--github-token GITHUB_TOKEN] [--github-push]
                             [--project-name PROJECT_NAME] [--repo-path REPO_PATH]
                             [--metadata-filename METADATA_FILENAME]
                             [--skipfile SKIPFILE] [--squad-host SQUAD_HOST]
...
optional arguments:
  ...
  --username USERNAME   The username for the test-definitions repo
